...
p = 0.25
alpha = 0.01
save_path = "./weights/HGNN_with_arcface.pt"

model = HGNN(num_classes, num_features, num_heads, hidden, p = p, alpha = alpha)
...
plt.ylabel('loss')
plt.ylim(0.5,4)
plt.savefig("./result/train_loss_curve_HGNN_with_arcface.png")
...
plt.ylabel('Accuracy')
plt.ylim(0, 1.0)
plt.savefig("./result/train_accuracy_curve_HGNN_with_arcface.png")
...
evaluate(model, test_loader, criterion, device, save_dir = "./result/test_summary_HGNN_with_arcface.txt")