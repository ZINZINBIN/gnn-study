...
alpha = 0.01
p = 0.5
save_path = "./weights/GConvNet_without_arcface.pt"

model = GConvNet(num_features, num_classes, hidden, alpha, p)
...
plt.ylabel('loss')
plt.ylim(0.5,4)
plt.savefig("./result/train_loss_curve_GConvNet_without_arcface.png")
...
plt.ylabel('Accuracy')
plt.ylim(0, 1.0)
plt.savefig("./result/train_accuracy_curve_GConvNet_without_arcface.png")
...
evaluate(model, test_loader, criterion, device, save_dir = "./result/test_summary_GConvNet_without_arcface.txt")